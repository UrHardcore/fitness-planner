...
const resetBtnTimer = qs('#resetBtn');
...
emailLoginBtn.addEventListener('click', async (e)=>{
  ...
  }catch(err){ authMsg.textContent = mapAuthError(err); }
});
...
emailRegisterBtn.addEventListener('click', async ()=>{
  try{
    ...
    if(pass.length < 6) return authMsg.textContent = 'La contraseña debe tener al menos 6 caracteres.';
    const cred = await createUserWithEmailAndPassword(auth, email, pass);
    // set displayName si no existe
...
resetBtn.addEventListener('click', async ()=>{
  const email = emailInput.value.trim();
  if(!email) return authMsg.textContent = 'Escribí tu email para enviarte el enlace.';
  try{
...
onAuthStateChanged(auth, async (user)=>{
  ...
  // Rol
  const rSnap = await get(ref(db, `roles/${user.uid}`)); // ¡ojo: roles en minúscula!
...
resetBtnTimer.addEventListener('click', ()=>{ clearInterval(timer); timer=null; elapsed=0; renderClock(); });
renderClock();
