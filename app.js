...
function applySavedTheme(){
  ...
  if (themeToggleLogin) themeToggleLogin.checked = saved==='dark';
  if (themeToggleApp) themeToggleApp.checked = saved==='dark';
}
...
/* ===== NUEVO: pedir y guardar “¿Cómo quieres que te llamen?” =====
   - Guarda en: users/{uid}/preferredName
   - Solo se pide si no existe; luego se usa siempre ese nombre.
*/
async function ensurePreferredName(user){
  try{
    ...
    // Si no existe el nodo del usuario, crear base
    if(!uSnap.exists()){
      await set(uRef, {
    ...
    // Si no hay preferredName, preguntar UNA vez (hasta que lo guarde)
    if (!data.preferredName) {
      const suggested = user.displayName || (user.email ? user.email.split('@')[0] : '');
      let name = window.prompt('¿Cómo quieres que te llamen?', suggested ?? '');
      if (name) {
        name = name.trim();
        if (name) {
          await update(uRef, { preferredName: name, updatedAt: Date.now() });
    ...
    // Devolver el nombre para mostrar
    return data.preferredName || user.displayName || (user.email ? user.email.split('@')[0] : 'Usuario');

  }catch(e){
    console.error(e);
    return user.displayName || (user.email ? user.email.split('@')[0] : 'Usuario');
  }
}
...
onAuthStateChanged(auth, async (user)=>{
  ...
  // === NUEVO: obtener/registrar nombre visible y mostrarlo ===
  const prettyName = await ensurePreferredName(user);
  userNameEl.textContent = prettyName;      // nombre arriba a la derecha
  currentStudent.value = prettyName;        // campo "Alumno"

  // Planner (igual que tenías)
  initWeeks();
  buildDayTabs();
  await loadWeekData();
...
async function loadWeekData(){
  ...
  currentStudent.value = (userInfo.val()?.preferredName || userInfo.val()?.displayName || userInfo.val()?.email || viewedUID);
